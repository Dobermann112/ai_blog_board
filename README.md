...
ユーザーが会員登録してブログ記事を投稿・公開できるサービスです。記事にはタグを付与でき、他ユーザーの記事に「お気に入り」登録やコメント投稿ができます。
...
- トップページ（記事一覧へのリンク）
- 記事一覧（新着順）・記事詳細（コメントの閲覧・投稿・編集・削除が可能。コメント5件以上はプルダウン表示）
- 記事の新規作成・編集（投稿者のみ）
- タグ一覧・タグ別記事一覧
- お気に入り一覧（自分がお気に入り登録した記事一覧）
- フォロー一覧（自分がフォローしているユーザーの一覧）
- ユーザー投稿一覧（特定ユーザーが投稿した公開記事一覧、フォロー登録もここから行える）
